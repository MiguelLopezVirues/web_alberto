...
export default function Footer() {
  return (
    <footer className="bg-ground-deep px-section-x-sm md:px-section-x relative z-[1]" aria-label="Pie de página">
      <div className="max-w-container mx-auto w-full flex flex-wrap items-center gap-4 py-[1.25rem] md:h-[60px] md:py-0">
        ...
        <ul className="flex gap-6 mx-auto list-none" role="list">
          {['Aviso legal', 'Privacidad', 'Cookies'].map(label => (
            <li key={label}>
              <a href="#" className="font-ui text-[0.75rem] text-white/35 hover:text-white/70 transition-colors duration-fast">
                {label}
              </a>
            </li>
          ))}
